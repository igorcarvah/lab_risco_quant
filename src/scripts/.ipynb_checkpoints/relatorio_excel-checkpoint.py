...
def gerar_relatorio_final():
    ...
    # --------------------------------------------------------------------------
    row_start = 6
    cols = ['Ativo', 'Setor', 'Retorno', 'Volat.', 'Sharpe', 'VaR 95%', 'Skew', 'Kurt', 'Max DD']
    ...
    for r_idx, row in enumerate(df.to_dict('records'), row_start + 1):
        ...
        for c in range(2, 11): 
            ...
            if cell.font.size is None: cell.font = Font(size=10)

    ultima_linha = row_start + len(df)

    # Barras de Dados
    ws.conditional_formatting.add(f'E{row_start+1}:E{ultima_linha}', DataBarRule(start_type='min', end_type='max', color="FF6384"))
    ws.conditional_formatting.add(f'F{row_start+1}:F{ultima_linha}', DataBarRule(start_type='min', end_type='max', color="36A2EB"))
    ...
    chart.style = 2; chart.height = 10; chart.width = 16
    ...
    ws.add_chart(chart, f"B{linha_grafico}")
    ...
    ws.merge_cells(start_row=linha_alerta, start_column=col_alerta, end_row=linha_alerta, end_column=col_alerta+2)
    ...
    termos = [
        ...
        ("Volatilidade", "Incerteza do preço (Desvio Padrão Anualizado)."),
        ("Sharpe Ratio", "Retorno por unidade de risco (acima do CDI)."),
        ("VaR 95%", "Perda máxima esperada em 95% dos dias."),
        ("Skewness", "Viés da curva (Negativo = Risco de quedas abruptas)."),
        ("Kurtosis", "Risco de Cauda (Cisne Negro). > 3 é sinal de alerta."),
        ("Max DD", "Máximo declínio do topo ao fundo histórico.")
    ]
